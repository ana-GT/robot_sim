...
namespace gazebo
{
	...
	for (unsigned int i = 0; i < this->joints.size(); ++i) {
	    ...
	    double positionTarget = math::clamp( this->jointCommands.position[i],
	    ...
	    // Damping
	    double jointDampingCoef = math::clamp( static_cast<double>( this->drchuboState.kp_velocity[i] ),
						    this->jointDampingModel[i], 
						    this->jointDampingMax[i] );
	    // Skip set joint damping if value is not changing
	    if( !math::equal( this->lastJointCFMDamping[i],
			      jointDampingCoef ) ) {
	    ...
	    // this is the approximate force of the infinite bandwidth kp-velocity term
	    double kpVelocityDampingEffort = 0;
	    double kpVelocityDampingCoef = jointDampingCoef - this->jointDampingModel[i];
	    if( kpVelocityDampingCoef > 0.0 ) {
		kpVelocityDampingEffort = kpVelocityDampingCoef * this->drchuboState.velocity[i];
	    }
	    ...
	    // note: We do not use k_effort since we don't have BDI (para lo que sirve... :D)
	    double forceUnclamped = ( this->drchuboState.kp_position[i] * this->errorTerms[i].q_p +
				      this->errorTerms[i].k_i_q_i +
				      this->drchuboState.kd_position[i] * this->errorTerms[i].d_q_p_dt +
				      jointDampingCoef * this->jointCommands.velocity[i] +
